...
function NoteCard({ note, onOpen, onToggleArchive, onDelete }) {
  return (
    <div
      role="button"
      tabIndex={0}
      ...
      style={{
        background: "var(--bg-card)",
        border: `1px solid ${note.isArchived ? "var(--accent)" : "var(--border-color)"}`,
        borderRadius: 10,
        padding: 18,
        cursor: "pointer",
...
export default function App() {
  ...
  function addNote({ title, text }) {
    setNotes((prev) => [...prev, { id: Date.now(), title, text, isArchived: false }]);
  }
